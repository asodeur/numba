...
class Lower(BaseLower):
    ...
    def lower_binop(self, resty, expr, op):
        ...
        casted_lhs = self.context.cast(
            self.builder, lhs, lty, signature.args[0])
        casted_rhs = self.context.cast(
            self.builder, rhs, rty, signature.args[1])

        def cast_result(res):
            # ATTN: decrefs res, casted_lhs, and casted_rhs.
    ...
    def lower_expr(self, resty, expr):
        if expr.op == 'binop':
            ...
            # Convert argument to match
            castval = self.context.cast(
                self.builder, val, typ, signature.args[0])
